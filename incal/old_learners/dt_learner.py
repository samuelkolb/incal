...
class OCTLearner(Learner):
    ...
    def learn(self, domain, data):
        ...
        Y_ik = [[1 if data[i][1] else -1, -1 if data[i][1] else 1] for i in range(n)]
        ...
        L_t = [
            m.addVar(vtype=GRB.INTEGER, name="L_{}".format(i))
            for i in range(leaf_count)
        ]
        s_jt = [
            [
                m.addVar(vtype=GRB.BINARY, name="s{}_{}".format(j, i))
                for i in range(inline_count)
            ]
            for j in range(p)
        ]
        N_t = [
            m.addVar(vtype=GRB.INTEGER, name="N_{}".format(i))
            for i in range(leaf_count)
        ]
        N_kt = [
            [
                m.addVar(vtype=GRB.INTEGER, name="N_{}_{}".format(k, i))
                for i in range(leaf_count)
            ]
            for k in range(K)
        ]
        c_kt = [
            [
                m.addVar(vtype=GRB.BINARY, name="c_{}_{}".format(k, i))
                for i in range(leaf_count)
            ]
            for k in range(K)
        ]
        z_it = [
            [
                m.addVar(vtype=GRB.BINARY, name="z_{}_{}".format(i, t))
                for t in range(leaf_count)
            ]
            for i in range(n)
        ]
        l_t = [
            m.addVar(vtype=GRB.BINARY, name="l_{}".format(t)) for t in range(leaf_count)
        ]
        a_jt = [
            [
                m.addVar(vtype=GRB.CONTINUOUS, lb=-1, ub=1, name="a_{}_{}".format(j, t))
        ...
        a_hat_jt = [
            [
                m.addVar(vtype=GRB.CONTINUOUS, name="a_hat_{}_{}".format(j, t))
                for t in range(inline_count)
            ]
            for j in range(p)
        ]
        b_t = [
            m.addVar(vtype=GRB.CONTINUOUS, name="b_{}".format(t))
            for t in range(inline_count)
        ]
        d_t = [
            m.addVar(vtype=GRB.BINARY, name="d_{}".format(t))
            for t in range(inline_count)
        ]
        print()
        ...
        # Set objective
        m.setObjective(
            1.0 / L_hat * sum(L_t) + self.alpha * sum(sum(entry) for entry in s_jt),
            GRB.MINIMIZE,
        )
        ...
        for k in range(K):
            for t in range(leaf_count):
                name = (
                    "N_{k}_{t} = 1/2 * SUM_(i = 0..{n}) (1 + Y_i_{k}) * z_i_{t}".format(
                        k=k, t=t, n=n - 1
                    )
                )
                m.addConstr(
                    N_kt[k][t]
                    == sum((1 + Y_ik[i][k]) * z_it[i][t] for i in range(n)) / 2,
                    name,
                )
                print(name)
        print()
        ...
        for i in range(n):
            for t in range(leaf_count):
                for _m in A_L[inline_count + t]:
                    ...
                    name = "SUM_(j = 0..{p}) (a_j_{m}^T * x_j_{i}) + mu <= b_{m} + (2 + mu) * (1 - z_{i}_{t})".format(
                        i=i, t=t, m=_m, p=p - 1
                    )
                    m.addConstr(
                        vector_product + self.mu
                        <= b_t[_m] + (2 + self.mu) * (1 - z_it[i][t]),
                        name,
                    )
                    print(name)
        print()
        ...
        for i in range(n):
            for t in range(leaf_count):
                for _m in A_R[inline_count + t]:
                    name = "SUM_(j = 0..{p}) a_j_{m}^T * x_j_{i} >= b_{m} - 2 * (1 - z_{i}_{t})".format(
                        i=i, t=t, m=_m, p=p - 1
                    )
                    vector_product = sum(a_jt[j][_m] * x_ij[i][j] for j in range(p))
        ...
        for i in range(n):
            name = "SUM_(t in {T_L}) z_{i}_t = 1".format(
                i=i, T_L=list(range(leaf_count))
            )
