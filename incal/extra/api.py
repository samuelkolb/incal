...
if __name__ == "__main__":
    ...
    def parse_args():
        ...
        scan_parser = subparsers.add_parser(
            "scan", help="Scan the directory and load smt problems"
        )
        scan_parser.add_argument(
            "-d", "--dir", default=None, help="Specify the directory to load files from"
        )
        ...
        learn_parser.add_argument(
            ...
            "--samples",
            type=int,
            help="Specify the number of samples for learning",
        )
        learn_parser.add_argument(
            "-a",
            "--all",
        ...
        table_parser = subparsers.add_parser(
            "table", help="Types can be: [time, k, h, id, acc, samples, l]"
        )
        table_parser.add_argument("row_key", help="Specify the row key type")
        table_parser.add_argument(
            "col_key", default=None, help="Specify the col key type"
        )
        table_parser.add_argument("value", default=None, help="Specify the value type")
        table_parser.add_argument(
            "dirs",
            nargs="*",
            help="Specify the directories to load files from, always in pairs:"
            "result_dir, data_dir",
        )

        table_subparsers = table_parser.add_subparsers(dest="command")
        table_print_parser = table_subparsers.add_parser(
            "print", help="Print the table"
        )
        table_print_parser.add_argument(
            "-d",
            "--delimiter",
            default="\t",
            help="Specify the delimiter (default=tab)",
        )
        table_print_parser.add_argument(
            "-a",
            "--aggregate",
        ...
        table_plot_parser.add_argument(
            "-a",
            "--aggregate",
            default=False,
            action="store_true",
        ...
        table_plot_parser.add_argument(
            "--y_min", default=None, type=float, help="Minimum value for y"
        )
        table_plot_parser.add_argument(
            "--y_max", default=None, type=float, help="Maximum value for y"
        )
        table_plot_parser.add_argument(
            "--x_min", default=None, type=float, help="Minimum value for x"
        )
        table_plot_parser.add_argument(
            "--x_max", default=None, type=float, help="Maximum value for x"
        )
        table_plot_parser.add_argument(
            "--legend_pos", default=None, type=str, help="Legend position"
        )
        table_plot_parser.add_argument(
            "-o", "--output", default=None, help="Specify the output file"
        )

        combine_parser = subparsers.add_parser(
            "combine", help="Combine multiple results directories"
        )
        combine_parser.add_argument(
            "output_dir", help="The output directory to summarize results in"
        )
        combine_parser.add_argument(
            "input_dirs", nargs="*", help="Specify the directories to combine"
        )
        combine_parser.add_argument(
            "-b", "--bias", default=None, help="Specify the bias"
        )
        combine_parser.add_argument(
            "-p", "--prefix", default=None, help="Specify the prefix for input dirs"
        )

        gen_parser = subparsers.add_parser(
            "generate", help="Generate synthetic examples"
        )
        ...
        migration_parser = subparsers.add_parser(
            "migrate", help="Migrate files to newer or extended versions"
        )
        migration_subparsers = migration_parser.add_subparsers(dest="type")

        migration_fix_parser = migration_subparsers.add_parser(
            "fix", help="Fix result files"
        )
        migration_fix_parser.add_argument(
            "results_dir", help="Specify the result directory"
        )
        migration_fix_parser.add_argument(
            "-b", "--bias", default=None, help="Specify the bias"
        )

        migration_acc_parser = migration_subparsers.add_parser(
            "accuracy", help="Add accuracy to result files"
        )
        migration_acc_parser.add_argument(
            "results_dir", help="Specify the result directory"
        )
        migration_acc_parser.add_argument(
            "-d", "--data_dir", help="Specify the data directory for synthetic problems"
        )
        migration_acc_parser.add_argument(
            "-s",
            "--samples",
        ...
        migration_acc_parser.add_argument(
            ...
            default=False,
            action="store_true",
            help="Overwrites existing values",
        )

        migration_ratio_parser = migration_subparsers.add_parser(
            "ratio", help="Add ratio to result files"
        )
        migration_ratio_parser.add_argument(
            "results_dir", help="Specify the result directory"
        )
        migration_ratio_parser.add_argument(
            "-d", "--data_dir", help="Specify the data directory for synthetic problems"
        ...
        if args.mode == "scan":
            ...
            for i in range(int(math.floor(len(args.dirs) / 3))):
                table.load_table(
                    args.dirs[3 * i], args.dirs[3 * i + 1], args.dirs[3 * i + 2]
                )
            ...
            combine_results.combine(
                args.output_dir, args.input_dirs, args.bias, args.prefix
            )
            ...
            if args.type == "fix":
                ...
                migrate.add_accuracy(
                    args.results_dir, args.data_dir, args.samples, args.force
                )
